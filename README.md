...
>
> **前提:** MFA の受信先メールアドレスとして **Gmail アドレスを登録している** こと

---

## 📨 MFA 受信先メールについて

京大の MFA 設定では、OTP の送付先として **京大以外のメールアドレス** を登録する必要があります（`@kyoto-u.ac.jp` / `@st.kyoto-u.ac.jp` は登録不可）。

この拡張機能は **Gmail API を使って OTP を読み取る** ため、MFA 受信先として **Gmail アドレスを登録している場合のみ** そのまま使えます。

Yahoo メール・Outlook.com など Gmail 以外を登録している場合は、そのアカウントから Gmail へ転送設定をするか、MFA の受信先を Gmail に変更してください。
...
MFA の受信先として登録している Gmail アカウントでサインインします。

1. 拡張機能アイコン（パズルピース）→ 「京大KULMS MFA自動入力」をピン留め
2. ピン留めしたアイコンをクリック → ポップアップが開く
3. 「**Gmailにサインイン**」をクリック → Google の認証画面で **MFA受信先の Gmail アカウント** を選んで承認
...
- Gmail 以外のメールプロバイダには未対応（MFA受信先が Yahoo・Outlook.com 等の場合は Gmail への転送が必要）
- メール配送の遅延が大きい場合は届く前にタイムアウトする可能性あり
